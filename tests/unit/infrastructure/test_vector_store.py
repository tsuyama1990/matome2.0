...
async def test_search_similar_connection_error(
    ...
    with pytest.raises(ConnectionError, match=f"{ERR_PINECONE_SEARCH_01}: Network down"):
...
async def test_upsert_chunks_retry_failure(
    ...
    with pytest.MonkeyPatch().context() as m:
        m.setattr(asyncio, "sleep", AsyncMock())
        with pytest.raises(ConnectionError, match=f"{ERR_PINECONE_UPSERT_01}: Network down"):
            await vector_client.upsert_chunks([chunk])
...
async def test_search_similar_invalid_query(test_config: PineconeConfig) -> None:
    ...
    with pytest.raises(
        ValueError, match=ERR_EMBEDDING_MUST_BE_LIST
    ):
        await client.search_similar(None, top_k=5) # type: ignore

    with pytest.raises(
        ValueError, match=ERR_EMBEDDING_CANNOT_BE_EMPTY
    ):
        await client.search_similar(TestDataFactory.get_empty_list(), top_k=5)

    with pytest.raises(
        ValueError, match=ERR_EMBEDDING_MUST_BE_NUMERIC
    ):
...
@pytest.mark.asyncio
async def test_search_similar_handles_dict_response(test_config: PineconeConfig):
    mock_index = MagicMock(spec=PineconeIndexProtocol)
    client = PineconeClient(
        index=mock_index,
        config=test_config,
...
@pytest.mark.asyncio
async def test_search_similar_handles_missing_matches(test_config: PineconeConfig):
    mock_index = MagicMock(spec=PineconeIndexProtocol)
    client = PineconeClient(
        index=mock_index,
        config=test_config,
...
@pytest.mark.asyncio
async def test_search_similar_handles_object_with_none_matches(test_config: PineconeConfig):
    mock_index = MagicMock(spec=PineconeIndexProtocol)
    client = PineconeClient(
        index=mock_index,
        config=test_config,
...
@pytest.mark.asyncio
async def test_search_similar_handles_object_missing_attributes(test_config: PineconeConfig):
    mock_index = MagicMock(spec=PineconeIndexProtocol)
    client = PineconeClient(
        index=mock_index,
        config=test_config,
...
def test_pinecone_index_adapter_upsert_exception():
    ...
    adapter = PineconeIndexAdapter(mock_index)
    with pytest.raises(ConnectionError, match=f"{ERR_PINECONE_ADAPTER_UPSERT_01}: failed"):
        adapter.upsert([])
...
def test_pinecone_index_adapter_query_exception():
    ...
    adapter = PineconeIndexAdapter(mock_index)
    with pytest.raises(ConnectionError, match=f"{ERR_PINECONE_ADAPTER_QUERY_01}: failed"):
...
def test_pinecone_index_adapter_describe_stats_exception():
    ...
    mock_index.describe_index_stats.side_effect = Exception("failed")
    adapter = PineconeIndexAdapter(mock_index)
    with pytest.raises(ConnectionError, match=f"{ERR_PINECONE_ADAPTER_STATS_01}: failed"):
...
async def test_upsert_chunks_invalid_metadata(vector_client: PineconeClient) -> None:
    chunk = TestDataFactory.create_mock_chunk()
    chunk = chunk.model_copy(update={"metadata": {"valid": "ok", "invalid": [], "text": "should be dropped"}})
    await vector_client.upsert_chunks([chunk])
...
def test_pinecone_config_post_init_validation():
    with pytest.raises(ValueError, match="max_retries must be positive"):
        PineconeConfig(max_retries=0, base_delay=1.0, batch_size=1, max_batch_size=1)
    with pytest.raises(ValueError, match="base_delay must be positive"):
        PineconeConfig(max_retries=1, base_delay=0.0, batch_size=1, max_batch_size=1)
    with pytest.raises(ValueError, match="batch_size must be positive"):
        PineconeConfig(max_retries=1, base_delay=1.0, batch_size=0, max_batch_size=1)
    with pytest.raises(ValueError, match="max_batch_size must be positive"):
        PineconeConfig(max_retries=1, base_delay=1.0, batch_size=1, max_batch_size=0)

@pytest.mark.asyncio
async def test_upsert_chunks_invalid_chunks_type(vector_client: PineconeClient) -> None:
    with pytest.raises(TypeError, match="chunks must be a list"):
        await vector_client.upsert_chunks("not-a-list") # type: ignore

@pytest.mark.asyncio
async def test_upsert_chunks_invalid_chunk_item_type(vector_client: PineconeClient) -> None:
    with pytest.raises(TypeError, match="All items in chunks must be DocumentChunk instances"):
        await vector_client.upsert_chunks(["not-a-chunk"]) # type: ignore


def test_vector_store_factory_invalid_index():
    from src.infrastructure.vector_store import VectorStoreFactory
    with pytest.raises(ValueError, match="index cannot be None"):
        VectorStoreFactory.create_pinecone_client(None, PineconeConfig(1,1,1,1)) # type: ignore

def test_vector_store_factory_invalid_config():
    from src.infrastructure.vector_store import VectorStoreFactory
    with pytest.raises(ValueError, match="config cannot be None"):
        VectorStoreFactory.create_pinecone_client(MagicMock(), None) # type: ignore
